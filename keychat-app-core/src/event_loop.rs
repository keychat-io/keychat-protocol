//! Event loop — incoming event processing, friend request handling, message routing.
...
use crate::app_client::{lock_app_storage, default_device_id, AppClient, AppError, AppResult};
// Import app-core types but disambiguate from libkeychat re-exports
use crate::types::{
    self, ClientEvent, ConnectionStatus, DataChange, MessageKind, MessageStatus,
    RoomStatus, RoomType, make_room_id,
    GroupChangeKind as AppGroupChangeKind,
};
...
impl AppClient {
            ...
            loop {
                tokio::select! {
                    _ = timeout_stop_rx.changed() => {
                        ...
                        let mut tracker = self_timeout.relay_tracker
                            .lock()
    ...
    async fn try_handle_friend_request(&self, event: &Event) -> bool {
        ...
        if !identity_pubkey.is_empty() {
            ...
            let sender_npub = crate::app_client::npub_from_hex(sender_pubkey.clone()).unwrap_or_default();
            ...
            let existing_room_status = {
                ...
                let store = crate::app_client::lock_app_storage(&app_storage);
            ...
            if let Some(Ok((msg, decrypt_result))) = result {
                if msg.kind == KCMessageKind::FriendApprove {
                    ...
                    // ── Persist to app_* tables ──────────────────
                    {
                        ...
                        if !identity_pubkey.is_empty() {
                            let peer_npub =
                                crate::app_client::npub_from_hex(peer_nostr_id.clone()).unwrap_or_default();
                    ...
                    if !identity_pubkey.is_empty() {
                        ...
                        {
                            let store = crate::app_client::lock_app_storage(&inner.app_storage);
    ...
    async fn try_handle_nip17_dm(
        ...
        let sender_npub = crate::app_client::npub_from_hex(sender_pubkey.clone())
                ...
                match msg.kind {
                                ...
                                // Persist group room to app_* tables
                                {
                                    ...
                                    if !identity_pubkey.is_empty() {
                                        let saved = {
                                            ...
                                            let store =
                                                crate::app_client::lock_app_storage(&app_storage);
                                            store
                                                .save_app_room(
                                                    &group_id,
                                                    &identity_pubkey,
                    ...
                    KCMessageKind::SignalGroupMemberRemoved => {
                        ...
                        self.emit_event(ClientEvent::GroupMemberChanged {
                            room_id: group_id,
                            kind: AppGroupChangeKind::MemberRemoved,
                    ...
                    KCMessageKind::SignalGroupSelfLeave => {
                        ...
                        self.emit_event(ClientEvent::GroupMemberChanged {
                            room_id: group_id,
                            kind: AppGroupChangeKind::SelfLeave,
                    ...
                    KCMessageKind::SignalGroupDissolve => {
                        ...
                        {
                            let full_room_id = crate::types::make_room_id(&group_id, &identity_pubkey);
                            let app_store = crate::app_client::lock_app_storage(&app_storage_clone);
                    ...
                    KCMessageKind::SignalGroupNameChanged => {
                        ...
                        self.emit_event(ClientEvent::GroupMemberChanged {
                            room_id: group_id,
                            kind: AppGroupChangeKind::NameChanged,
                            member_pubkey: None,
                            new_value: new_name,
                        })
                        .await;
    ...
    // emit_event and emit_data_change are defined in app_client.rs

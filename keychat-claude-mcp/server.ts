 ...
 *
 * Architecture:
 *   Claude Code ←stdio MCP→ this MCP server ←HTTP→ keychat-cli daemon ←Nostr→ Relay
 *
 * The daemon handles all encryption (Signal PQXDH + NIP-17 gift-wrap).
 * This MCP server only does MCP ↔ HTTP translation.
 */
  ...
  try {
    switch (name) {
      ...
      case 'fetch_messages': {
        ...
        const res = await daemonGet(`/rooms/${chatId}/messages?limit=${limit}`);
        if (!res?.ok) {
      ...
      case 'list_contacts': {
        ...
        const formatted = contacts
          .map((c: any) => {
            const name = c.petname || c.name || c.pubkey?.slice(0, 16) || '?';
...
async function main() {
  ...
  console.error('[keychat] Claude MCP server started');

...
impl MultiAgentManager {
    ...
    pub async fn create_agent(&self, agent_id: &str, name: &str) -> Result<String> {
        ...
        config::store_mnemonic(&pubkey_hex, &gen.mnemonic)?;
        let db_key = config::generate_db_key();
        config::store_db_key(&pubkey_hex, &db_key)?;
    ...
    async fn start_agent(&self, agent_id: &str) -> Result<()> {
        ...
        let config = Config::load(&agent_dir)?
            .ok_or_else(|| anyhow::anyhow!("No config.json in agent '{}'", agent_id))?;
    ...
    pub async fn list_agents(&self) -> Vec<AgentInfo> {
        self.agents.read().await.values().map(|h| AgentInfo {
            id: h.id.clone(),
            npub: h.npub.clone(),
            name: h.name.clone(),
        }).collect()

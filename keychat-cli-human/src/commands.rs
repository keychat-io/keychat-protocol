...
pub async fn handle(state: &AppState, input: &str) -> anyhow::Result<bool> {
    ...
    match cmd {
        ...
        // ── 1:1 Chat ────────────────────────────────────────────────
        "/add" => {
            if arg.is_empty() { ui::err("Usage: /add <npub_hex>"); }
            else { chat::add_friend(state, arg).await?; }
        }

        "/chat" => {
            if arg.is_empty() {
                ui::err("Usage: /chat <npub_hex>");
            } else {
                *state.active_chat.write().await = Some(ChatTarget::Peer(arg.to_string()));
                let name = state.peers.read().await.get(arg)
                    .map(|p| p.name.clone())
                    .unwrap_or_else(|| format!("{}...", &arg[..8.min(arg.len())]));

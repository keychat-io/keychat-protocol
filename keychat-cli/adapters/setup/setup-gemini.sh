#!/usr/bin/env bash
#
# Keychat Agent + Gemini CLI — one-click installer
#
# Usage:
...
#   4. Writes Gemini CLI MCP config (~/.gemini/settings.json)
...
echo -e "${BOLD}Keychat Agent + Gemini CLI Setup${NC}"
echo ""
...
# ─── Step 4: Configure Gemini CLI MCP ───────────────────────
GEMINI_DIR="$HOME/.gemini"
SETTINGS_FILE="$GEMINI_DIR/settings.json"
SERVER_TS="$MCP_DIR/server.ts"

info "Configuring Gemini CLI MCP..."
mkdir -p "$GEMINI_DIR"

if [[ -f "$SETTINGS_FILE" ]]; then
  node -e "
    const fs = require('fs');
    const cfg = JSON.parse(fs.readFileSync('$SETTINGS_FILE', 'utf-8'));
...
info "Gemini config: $SETTINGS_FILE"
...
if $INSTALL_ONLY; then
  ...
  echo ""
  echo "  3. Configure the API token in MCP config"
  echo ""

...
impl KeychatClient {
    pub async fn send_text(
        ...
        let (session_mutex, peer_signal_hex, nostr_client) = {
            ...
            let signal_hex = inner.peer_nostr_to_signal.get(&room_id)
                .ok_or(KeychatUniError::PeerNotFound { peer_id: room_id.clone() })?
                .clone();
            let session = inner.sessions.get(&signal_hex)
                .ok_or(KeychatUniError::PeerNotFound { peer_id: signal_hex.clone() })?
                .clone(); // Arc clone — cheap
            let client = inner.transport.as_ref()
